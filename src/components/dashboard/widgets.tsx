...
function Kpi({
  ...
  const dirColor =
    delta?.dir === "down" ? "text-good" : delta?.dir === "up" ? "text-warn" : "text-ink-3";
...
function Nudge({ text }: { text: string }) {
  return (
    <div className="mt-2 flex items-start gap-1.5 rounded-lg bg-primary/[0.06] px-2.5 py-1.5 text-xs text-ink-2">
      <Sparkles className="mt-0.5 h-3 w-3 shrink-0 text-primary-ink" />
      <span>{text}</span>
    </div>
  );
}
...
export function DashboardWidget({ id, data }: { id: string; data: DashboardData }) {
  ...
  const nudge = data.nudges?.find((n) => n.widget === id);
  ...
  switch (id) {
    ...
    case "weight":
      return (
        <div>
          ...
          />
          {nudge && <div className="px-4 pb-3"><Nudge text={nudge.text} /></div>}
        </div>
      );
    case "bodyfat":
      return (
    ...
    case "adaptiveInsight": {
      const brief = data.coachBrief;
      const toneStyles = {
        good: "border-good/30 bg-good-wash",
        warn: "border-warn/30 bg-warn-wash",
        info: "border-primary/20 bg-primary/[0.06]",
        crit: "border-crit/30 bg-crit-wash",
      };
      const toneText = {
        good: "text-good",
        warn: "text-warn",
      ...
      const moodAvatar = brief.mood === "great" ? "🟢" : brief.mood === "alert" ? "🟡" : "🔵";
      return (
        <div className="card overflow-hidden">
          {/* Coach header */}
          <div className="flex items-center gap-3 border-b border-border/60 bg-surface-2/50 px-4 py-3">
            <div className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-primary text-lg">{moodAvatar}</div>
            <div className="min-w-0">
              <div className="text-sm font-semibold text-ink">MetaCoach</div>
              <div className="truncate text-xs text-ink-3">{brief.greeting}</div>
            </div>
            <Sparkles className="ml-auto h-4 w-4 shrink-0 text-primary-ink" />
          </div>
          {/* Coach cards */}
          <div className="space-y-2.5 px-4 py-3">
            {brief.cards.length > 0 ? brief.cards.map((c, i) => (
              <div key={i} className={cn("rounded-xl border px-3 py-2.5", toneStyles[c.tone])}>
                <div className="flex items-center gap-2">
                  <span className="text-base leading-none">{c.emoji}</span>
                  <span className={cn("text-[0.7rem] font-bold uppercase tracking-wide", toneText[c.tone])}>{c.tag}</span>
                </div>
                <p className="mt-1 text-[0.8rem] leading-snug text-ink-2">{c.text}</p>
                {c.action && (
                  <div className="mt-1.5 text-xs font-semibold text-primary-ink">→ {c.action}</div>
                )}
              </div>
            )) : (
              <p className="py-2 text-sm text-ink-3">Veriler analiz ediliyor — öğün veya antrenman kaydı ekledikçe öneriler burada belirecek.</p>
            )}
          </div>
          {/* Protein bar */}
          {data.consumed.protein > 0 && (
            <div className="border-t border-border/60 px-4 py-2.5">
              <div className="flex items-center justify-between text-xs">
                <span className="font-medium text-ink-2">Protein</span>
                <span className="tabular-nums text-ink-3">{data.consumed.protein}g / {macroTarget.protein}g</span>
              </div>
              <div className="mt-1 h-2 overflow-hidden rounded-full bg-surface-2">
                <div
                  className={cn("h-full rounded-full transition-all", brief.proteinPct >= 100 ? "bg-good" : brief.proteinPct >= 70 ? "bg-primary" : "bg-warn")}
                  style={{ width: `${Math.min(100, brief.proteinPct)}%` }}
                />
              </div>
            </div>
          )}
          {/* Footer stats */}
          <div className="flex flex-wrap gap-2 border-t border-border/60 px-4 py-2.5 text-[0.65rem] text-ink-3">
            <span>TDEE {fmt(brief.summary.tdee)} kcal</span>
            <span>·</span>
            <span>Hedef {fmt(brief.summary.target)} kcal</span>
            <span>·</span>
            <span>Veri %{brief.summary.dataQuality}</span>
            <span>·</span>
            <span>{brief.summary.nDays} gün</span>
          </div>
        </div>
      );
    }
      ...
      return (
        <Panel title="Antrenman" right={<Dumbbell className="h-4 w-4 text-primary-ink" />}>
          {w.hasSession ? (
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="text-sm font-medium">{w.coachProgramName}</span>
                <span className="rounded-full bg-good-wash px-1.5 py-0.5 text-[10px] font-semibold text-good">Koç</span>
              </div>
              {w.todayWorkouts.length > 0 ? (
                <div className="mt-2 flex items-center gap-1.5 text-sm text-good">
                  <span>✓</span> {w.todayWorkouts[0]}
                </div>
              ) : (
                <a href="/workout" className="mt-2 inline-flex items-center gap-1.5 text-xs font-semibold text-primary-ink">
                  Bugün antrenman kaydet →
                </a>
              )}
              {nudge && <Nudge text={nudge.text} />}
            </div>
          ) : (
            <a href="/workout" className="block">
